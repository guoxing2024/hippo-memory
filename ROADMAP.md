...
- [x] 0.1.4/0.1.6 — lazy model load (faster `dsh web` startup), digest blank fix + fallback, `memory_maintain delete`, read-保障（recall/verify 先确保模型 ready）; 37 tests
- [x] core 0.1.6 / adapter 0.1.8 — explainable recall (`reason`/`eligible`/`bestSimilarity`/`threshold`/`nearMisses`), three score views (`similarity`/`score`/`relativeScore`), literal identifier matching (Top-1 10.5%→99.6% on 277 real queries), `memory_maintain duplicates` report, `superseded` on override, FACT:-prefix merge fix, lazy store open + `prune` empty-store sweep; 47 tests
